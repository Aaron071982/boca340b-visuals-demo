...
export default function DependencyDiagram() {
  ...
  useEffect(() => {
    ...
    fetch('/diagrams/architecture_dependency-graph.md_01.mmd')
      ...
      .catch((err) => {
        const errorMessage = err instanceof Error ? err.message : 'Failed to load dependency diagram';
        ErrorHandler.handleDiagramRenderError('dependencies', errorMessage);
  ...
  if (loading) {
    return <LoadingSpinner message="Loading dependency diagram..." />;
  }
